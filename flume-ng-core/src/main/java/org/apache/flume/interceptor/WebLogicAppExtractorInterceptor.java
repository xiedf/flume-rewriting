 ...
 *
 */
public class WebLogicAppExtractorInterceptor implements Interceptor {

  private static final Logger logger = LoggerFactory.getLogger(WebLogicAppExtractorInterceptor.class);
   ...
   */
  private WebLogicAppExtractorInterceptor(boolean preserveExisting, String key,
                                          String value) {
  ...
  public Event intercept(Event event) {
    ...
    int indexSC = body.indexOf("ServletContext");
    if(indexSC != -1){
      if (body.charAt(indexSC + "ServletContext".length()) == '-') {
        int indexStart = body.indexOf('-', indexSC);
        if (body.charAt(indexStart + 1) == '/') indexStart++;
        int indexEnd = body.indexOf('>', indexSC);
        newvalue = indexStart + 1 < indexEnd ? body.substring(indexStart + 1, indexEnd) : "null";
      } else {
        int indexStart = body.indexOf(':', indexSC);
        int indexEnd = body.indexOf(' ', indexSC);
        newvalue = body.substring(indexStart + 1, indexEnd);
      }
    }
    headers.put("app", newvalue);
    return event;
  }
  ...
  public static class Builder implements Interceptor.Builder {
    ...
    public Interceptor build() {
      ...
      return new WebLogicAppExtractorInterceptor(preserveExisting, key, value);
    }
